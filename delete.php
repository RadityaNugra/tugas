...
if(isset($_GET['id'])){
	...
	$id = $_GET['id'];
	$cek = mysqli_query($koneksi, "SELECT * FROM pelanggan WHERE id='$id'") or die(mysqli_error($koneksi));
	if(mysqli_num_rows($cek) > 0){
		
		$del = mysqli_query($koneksi, "DELETE FROM pelanggan WHERE id='$id'") or die(mysqli_error($koneksi));
		if($del){
